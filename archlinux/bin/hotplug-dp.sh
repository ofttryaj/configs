...
read STATUS_DP < /sys/class/drm/card0-Virtual-1/status
read STATUS_DP2 < /sys/class/drm/card0-Virtual-2/status
read STATUS_DP3 < /sys/class/drm/card0-Virtual-3/status
read STATUS_DP4 < /sys/class/drm/card0-Virtual-4/status
read STATUS_HDMI < /sys/class/drm/card0-HDMI-A-1/status
read STATUS_HDMI2 < /sys/class/drm/card0-HDMI-A-2/status
export DISPLAY=:0
export XAUTHORITY=/home/wg/.Xauthority
...
if [ "$STATUS" = "disconnected" ]; then
	...
	if [[ $1 == "mirror" ]]; then
		/usr/bin/xrandr --output $DEV --mode 1280x800
		/usr/bin/xrandr --output eDP-1 --mode 1280x800 --same-as $DEV
	else
		...
		sleep 1
		nitrogen --head=0 --set-zoom ~wg/.backgrounds/"$background_primary".jpg
		nitrogen --head=1 --set-zoom ~wg/.backgrounds/"$background_secondary".jpg
	fi
	/usr/bin/xset -dpms
	/usr/bin/xset s off
fi
...
/usr/bin/sudo -E -u wg env HOME=~wg ~wg/.config/polybar/launch.sh
